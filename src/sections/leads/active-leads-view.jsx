...
import {
  ...
  Button, Select, MenuItem as MuiMenuItem, FormControl, InputLabel, Alert
} from '@mui/material';
...
export function ActiveLeadsListView() {
  ...
  const fetchActiveLeads = useCallback(
    async (page = table.page, rowsPerPage = table.rowsPerPage, searchTerm = search) => {
      try {
        setLoading(true);
        const response = await axiosInstance.get(endpoints.user.activeleads, {
          params: {
            page: page + 1,
        ...
        console.log('Active Leads Response:', response.data);
  ...
  const handleSearch = (e) => {
    ...
    fetchActiveLeads(0, table.rowsPerPage, value);
  };
...
function FollowUpModal({ open, onClose, lead }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTimeout(() => {setSuccessMsg('');resetForm();  
      onClose();     
      }, 1500);
    } catch (error) {
      ...
      <DialogContent>
        ...
        <Stack spacing={2} sx={{ mt: 1 }}>
          <DatePicker
            ...
            value={nextDate}
            onChange={(newVal) => setNextDate(newVal)}
...
function ReferredModal({ open, onClose, lead }) {
  ...
  const fetchEmployees = async (branchId) => {
    try {
      ...
      const res = await axiosInstance.get(
        endpoints.user.getemplbybranch(branchId)
      );
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await axiosInstance.post(endpoints.user.assignLead(lead.id), {
        branch_id: branch,
      ...
      console.error('Error saving referred:', error);
      setErrorMsg('Failed to save referred');
          ...
          <FormControl fullWidth>
            ...
            >
              {loading ? (
                ...
                branches.map((b) => (
                  <MuiMenuItem key={b.id} value={b.id}>
                    {b.branch_name}
                  </MuiMenuItem>
                ))
              )}
