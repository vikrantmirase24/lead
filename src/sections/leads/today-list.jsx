...
export function TodayLeadsListView() {
  ...
  const fetchTodaylead = useCallback(
    async (page = table.page, rowsPerPage = table.rowsPerPage, searchTerm = search) => {
      try {
        setLoading(true);
        const response = await axiosInstance.get(endpoints.user.todayleads, {
  ...
  useEffect(() => { fetchTodaylead(); }, [fetchTodaylead]);

  const handleSearch = (e) => {
    const value = e.target.value;
    setSearch(value);
    table.onChangePage(null, 0);
    fetchTodaylead(0, table.rowsPerPage, value);
  };
...
function ActiveLeadTableRow({ row, onFollowUp, onReferred }) {
  ...
  return (
    <>
      <TableRow hover>
        ...
        <TableCell>{row.official_contact_number}</TableCell>
      ...
      <CustomPopover open={popover.open} anchorEl={popover.anchorEl} onClose={popover.onClose}>
        <MenuList>
          ...
          <MenuItem onClick={() => { popover.onClose(); onFollowUp(); }}><Iconify icon="solar:calendar-bold" /> FollowUp</MenuItem>
          <MenuItem onClick={() => { popover.onClose(); onReferred(); }}><Iconify icon="solar:user-plus-bold" /> Referred</MenuItem>
        </MenuList>
      </CustomPopover>
    </>
  );
