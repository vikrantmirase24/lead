...
export function TodayFollowupLeadsView() {
  ...
  const fetchTodayFollowup = useCallback(
    async (page = table.page, rowsPerPage = table.rowsPerPage, searchTerm = search) => {
      try {
        setLoading(true);
        const response = await axiosInstance.get(endpoints.user.todayfollowup, {
          params: { page: page + 1, page_size: rowsPerPage, search: searchTerm || '' }
        });
        const leadsData = response.data?.results?.data || [];
        setLeads(leadsData);
        setTotalCount(response.data?.count || 0);
      } catch (error) {
        console.error('Error fetching today followup:', error);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => { fetchTodayFollowup(); }, [fetchTodayFollowup]);

  const handleSearch = (e) => {
    const value = e.target.value;
    setSearch(value);
    table.onChangePage(null, 0);
    fetchTodayFollowup(0, table.rowsPerPage, value);
  };
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <DashboardContent>
        <CustomBreadcrumbs
          heading="Today FollowUp Leads"
          links={[
            { name: 'Dashboard', href: paths.dashboard.root },
            { name: 'Today FollowUp Leads' },
            { name: 'View' },
          ]}
          sx={{ mb: { xs: 3, md: 3 } }}
        />
              ...
              <Table>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={TABLE_HEAD.length} align="center">No leads found.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
...
function TodayFollowupLeadTableRow({ row, onFollowUp, onReferred }) {
...
// FollowUp Modal (same as MonthLeads)
function FollowUpModal({ open, onClose, lead }) {
  ...
  const handleSave = async () => {
    ...
    } catch (error) { setErrorMsg('Error saving follow up'); setTimeout(() => setErrorMsg(''), 2000); }
...
// Referred Modal (same as MonthLeads)
          ...
          <FormControl fullWidth>
            ...
            <Select value={employee} onChange={(e) => setEmployee(e.target.value)} label="Employee" disabled={!branch}>
              {employeeLoading ? <MuiMenuItem disabled>Loading...</MuiMenuItem> :
                employees.length > 0 ? employees.map(emp => <MuiMenuItem key={emp.id} value={emp.id}>{emp.full_name}</MuiMenuItem>) :
